...
{
    char buff[BUFSIZ], filename[BUFSIZ], dir_name[BUFSIZ], usr_id[BUFSIZ];
    int pick = 0;
    do {        //define pick instead of choice for menu switch case 1, 2, 3
        printf("Which file you want to process?\n");
        printf("Enter 1 to pick the largest file\n");
        scanf("%d", &pick, large_file);        //pick 1 for largest file, recieves user input
        printf("Enter 2 to pick the smallest file\n");
        scanf("%d", &pick, small_file);         //pick 2 for smallest file, recieves user input
        printf("Enter 3 to specify the name of a file\n\n");
        scanf("%d", &pick, filename);//pick 3 for specific file, recieves user input
